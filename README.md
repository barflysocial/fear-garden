...
- No host dashboard controls on `/play`
...
- Host dashboard and player gameplay UI
...
The included `render.yaml` is configured to deploy immediately. For paid live commercial use, choose the Render service and database plans you want in your Render dashboard. The code supports PostgreSQL, but production uptime, backups, custom domain, and scaling are controlled by your hosting plan.
...
- `/play` does not show create-game controls, host dashboard controls, reset controls, remove-player controls, phase controls, host hidden stats, or links back to `/host`.
