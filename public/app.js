...
function clearActive(){ stopPolling(); state.view='home'; state.code=''; state.hostKey=''; state.playerId=''; state.data=null; state.openPanel=null; state.playerRenderKey=null; renderHome(); }
...
function playerStructureKey(d){
  ...
  const roundChoice=p.choices?.[s.currentRound]?.choiceType || p.choices?.[s.currentRound]?.choiceLabel || '';
  return JSON.stringify({
    ...
    alliances:(d.alliances||[]).map(a=>a.id).sort(),
...
function updatePlayerLive(d){
  ...
  setText('playerRoundTitle',s.currentRound?`Round ${s.currentRound}${r?': '+r.title:''}`:'Lobby');
...
function renderPlayer(){
  ...
  app.innerHTML=html`<section class="app-shell" data-player-shell><div class="panel"><div class="header"><div><div class="kicker" id="playerKicker">${esc(p.alias)} · ${esc(e.title)}</div><h2 id="playerRoundTitle">${s.currentRound?`Round ${s.currentRound}${r?': '+esc(r.title):''}`:'Lobby'}</h2></div><div class="timer-box"><span id="playerPhaseLabel">${esc(phaseLabels[s.phase]||s.phase)}</span><strong id="playerCountdown">${esc(countdownText(s))}</strong></div></div>
