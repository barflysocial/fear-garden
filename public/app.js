...
function shouldShowEpisodeIntro(d) {
  return state.view === 'player' && !!d?.session?.code && d.session.phase === 'lobby' && !hasSeenIntro(d.session.code, state.view);
}
function renderEpisodeIntro(d) {
  const e = d.episode || {};
  const s = d.session || {};
  const img = introImages[e.id] || '';
  const roleLabel = 'You Joined the Episode';
  const helperText = 'Tap to open the lobby and wait for the host to begin.';
...
function renderHowToPlayInstructions(context = 'all') {
  ...
  const hostPanel = html`<div class="panel soft instruction-panel">
    ...
    <ol class="instruction-list">
      ...
      <li><strong>Monitor the game.</strong> The host sees all player stats, choices, private cards, public/private messages, group stats, hidden threat pressure, and round results.</li>
    ...
    <section class="hero">
      <div class="title-card">
        ...
        <div class="panel soft">
          ...
          <button class="btn" id="resumeBtn" type="button">Resume Saved Host Session</button>
  ...
  app.innerHTML = html`
    <section class="hero">
      <div class="title-card player-home-card">
        ...
        <div class="panel soft">
          <h2>Player Entry</h2>
          <p>Players only see the join flow, lobby, private cards, messages, choices, and results.</p>
  ...
  app.innerHTML = html`
    <section class="grid host">
      <div class="grid">
        <div class="panel">
          <div class="header">
            <div><div class="kicker">Host · ${esc(e.subtitle || '')}</div><h2>${esc(e.title)}</h2><p class="muted">${esc(s.venueName)} · Game Code <span class="session-code">${esc(s.code)}</span></p><p class="muted"><strong>Scheduled:</strong> ${esc(formatSchedule(s))}</p></div>
            <span class="status-pill phase-pill">${esc(phaseLabels[s.phase] || s.phase)}</span>
          </div>
          <p class="tagline">${esc(e.tagline)}</p>
          <div class="host-controls"><button class="btn primary" id="advanceBtn">${hostAdvanceLabel(d)}</button><button class="btn" id="copyBtn">Copy Join Link</button><button class="btn danger" id="resetBtn">Reset Session</button><button class="btn" id="homeBtn">Leave Host</button></div>
          <div class="divider"></div>
          ${renderJoinQrPanel(d)}
...
loadEpisodes().then(() => {
  const scannedCode = String(urlCode || '').trim().toUpperCase();
  if (scannedCode && routeMode === 'player') {
    const savedCode = String(localStorage.getItem('is_code') || '').trim().toUpperCase();
    const savedPlayerId = localStorage.getItem('is_playerId') || '';
    if (savedCode !== scannedCode || !savedPlayerId || localStorage.getItem('is_view') !== 'player') {
      localStorage.removeItem('is_view');
      localStorage.removeItem('is_code');
      localStorage.removeItem('is_playerId');
      state.view = 'home'; state.code = ''; state.playerId = ''; state.data = null;
      renderHome();
      setTimeout(() => { const input = document.querySelector('input[name="code"]'); if (input) input.value = scannedCode; }, 0);
      return;
    }
  }
  if (state.view === 'host' || state.view === 'player') startPolling(); else renderHome();
});
