...
function hostGameCard(g){
  return `<div class="host-game-card" data-host-game-card="${esc(g.code)}"><div class="host-game-card__top"><div><strong data-game-title>${esc(g.title)}</strong><small>${esc(g.venue)} · <span class="session-code">${esc(g.code)}</span></small></div><span class="status-pill" data-game-status>Checking…</span></div><div class="host-game-metrics"><span data-game-phase>Locked</span><span data-game-players>— players</span><strong data-game-countdown>—</strong></div><div class="host-controls"><button class="btn small" data-open-host="${esc(g.code)}">Open</button><button class="btn small" data-copy-host="${esc(g.code)}">Copy Link</button><button class="btn small" data-quick-host="${esc(g.code)}" hidden>Start</button><button class="btn danger small" data-delete-host="${esc(g.code)}">Delete</button></div></div>`;
}
async function refreshHostGameCards(){
  const games=hostGames();
  await Promise.all(games.map(async g=>{
    const card=document.querySelector(`[data-host-game-card="${CSS.escape(g.code)}"]`); if(!card)return;
    const hostKey=sessionStorage.getItem(`barfly_host_key_${g.code}`);
    const status=card.querySelector('[data-game-status]'), phase=card.querySelector('[data-game-phase]'), players=card.querySelector('[data-game-players]'), countdown=card.querySelector('[data-game-countdown]'), quick=card.querySelector('[data-quick-host]');
    if(!hostKey){ if(status)status.textContent='Locked'; if(phase)phase.textContent='Enter PIN to manage'; if(players)players.textContent='— players'; if(countdown)countdown.textContent='—'; if(quick)quick.hidden=true; return; }
    try{
      const d=await api(`/api/sessions/${encodeURIComponent(g.code)}/state?hostKey=${encodeURIComponent(hostKey)}`), s=d.session;
      if(status)status.textContent=s.status==='ended'?'Ended':s.paused?'Paused':s.status==='lobby'?'Scheduled':'Running';
      if(phase)phase.textContent=phaseLabels[s.phase]||s.phase;
      if(players)players.textContent=`${d.players.length}/${s.maxPlayers} players`;
      if(countdown)countdown.textContent=countdownText(s);
      if(quick){ quick.hidden=s.status==='ended'; quick.textContent=s.status==='lobby'?'Start':s.paused?'Resume':'Pause'; quick.dataset.quickAction=s.status==='lobby'?'advance':s.paused?'resume':'pause'; }
    }catch(err){ if(status)status.textContent='Unavailable'; if(phase)phase.textContent=err.message; if(quick)quick.hidden=true; }
  }));
}
function renderHostHome(){
  stopPolling();
  const cards=hostGames().map(hostGameCard).join('');
  app.innerHTML=html`<section class="app-shell"><div class="grid two">
    ...
    <div class="panel"><h2>My Games</h2><div class="players" id="hostGameList">${cards||'<p class="muted">No saved games on this device.</p>'}</div></div></div>
  </div></section>`;
  ...
  document.querySelectorAll('[data-copy-host]').forEach(b=>b.addEventListener('click',async()=>{const link=`${location.origin}/play?code=${b.dataset.copyHost}`;try{await navigator.clipboard.writeText(link);showToast('Join link copied.');}catch{showToast(link);}}));
  document.querySelectorAll('[data-quick-host]').forEach(b=>b.addEventListener('click',async()=>{
    const code=b.dataset.quickHost, hostKey=sessionStorage.getItem(`barfly_host_key_${code}`); if(!hostKey)return showToast('Open the game with its PIN first.');
    const action=b.dataset.quickAction;
    try{ if(action==='advance') await api(`/api/sessions/${code}/host/advance`,{method:'POST',body:{hostKey}}); else await api(`/api/sessions/${code}/host/automation`,{method:'POST',body:{hostKey,action}}); await refreshHostGameCards(); }catch(err){showToast(err.message);}
  }));
  refreshHostGameCards();
  state.poll=setInterval(refreshHostGameCards,3000);
}
...
function playerStructureKey(d){
  ...
  return JSON.stringify({
    ...
    role:p.role?.id||p.role?.name||p.role||'', card:p.privateCard?.id||'', cardDecision:p.privateCardDecision?.optionId||p.privateCardDecision?.type||p.privateCardDecision||'',
...
function infoPanelMarkup(d,panel=state.openPanel){
  ...
  if(panel==='players') return `<h2>Players</h2><div class="players">${d.players.map(p=>`<div class="player-row"><strong>${esc(p.firstName||p.alias)}</strong><small>${esc(p.role?.name||'Role hidden')} · ${esc(p.status)}</small></div>`).join('')}</div>`;
  return '';
}
...
function renderPledgeBoard(d){
  const rows=(d.publicPledges||[]).map(x=>`<div class="pledge-row"><strong>${esc(x.firstName||x.alias)}</strong><span>${esc(x.pledge?.pledgeLabel||'No pledge yet')}</span></div>`).join('');
...
function updatePlayerLive(d){
  ...
  setText('playerKicker',`${p.firstName||p.alias} · ${e.title}`);
...
function renderHostChoices(d){return `<div class="panel"><h2>Pledges & Choices</h2>${d.players.map(p=>{const privateDecision=p.privateCardDecisions?.[d.session.currentRound];return `<div class="player-row host-choice-row"><div><strong>${esc(p.firstName||p.alias)}</strong><small>Pledge: ${esc(p.pledges?.[d.session.currentRound]?.pledgeLabel||'None')}</small><small>Private decision: ${esc(privateDecision?.optionLabel||'Not locked')}</small></div><small>Final action: ${esc(p.choices?.[d.session.currentRound]?.choiceLabel||'Not locked')}</small></div>`;}).join('')}</div>`;}
...
function renderPlayer(){
  ...
  app.innerHTML=html`<section class="app-shell" data-player-shell><div class="panel"><div class="header"><div><div class="kicker" id="playerKicker">${esc(p.firstName||p.alias)} · ${esc(e.title)}</div><h2 id="playerRoundTitle">${esc(playerHeading(d))}</h2></div><div class="timer-box"><span id="playerPhaseLabel">${esc(phaseLabels[s.phase]||s.phase)}</span><strong id="playerCountdown">${esc(countdownText(s))}</strong></div></div>
...
function renderPrivateCard(d){
  const c=d.player.privateCard, chosen=d.player.privateCardDecision?.optionId || d.player.privateCardDecision?.type;
  if(!c) return '<div class="panel"><p>Preparing your private scenario.</p></div>';
  const options=Array.isArray(c.options)?c.options:[];
  return `<div class="panel private-scenario-panel"><div class="kicker">Private Scenario</div><h2>${esc(c.title)}</h2><p class="private-scenario-text">${esc(c.text)}</p>${c.targetName?`<p class="muted"><strong>Person involved:</strong> ${esc(c.targetName)}</p>`:''}<div class="private-choice-grid">${options.map(option=>`<button class="choice-card private-choice-card ${chosen===option.id?'selected':''}" data-card-option="${esc(option.id)}" ${chosen?'disabled':''}><div class="type">Your action</div><h3>${esc(option.label)}</h3><p>${esc(option.description||'')}</p></button>`).join('')}</div>${chosen?`<div class="goodbox private-choice-confirm"><strong>Decision locked:</strong> ${esc(d.player.privateCardDecision?.optionLabel||options.find(x=>x.id===chosen)?.label||'Selected')}</div>`:''}</div>`;
}
...
function bindPlayerEvents(d){const code=d.session.code,pid=d.player.id;
  document.querySelectorAll('[data-card-option]').forEach(b=>b.onclick=async()=>{try{await api(`/api/sessions/${code}/players/${pid}/private-card`,{method:'POST',body:{optionId:b.dataset.cardOption}});await fetchState();}catch(err){showToast(err.message);}});
