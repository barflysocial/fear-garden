...
async function api(path, options = {}) {
  ...
  if (!data || !res.ok || data.error) throw new Error((data && data.error) || 'Request failed.');
  return data;
}
...
function timeRemainingText(session) {
  if (!session?.phaseEndsAt) return '';
  const ms = new Date(session.phaseEndsAt).getTime() - new Date(session.serverNow || Date.now()).getTime();
  const total = Math.max(0, Math.ceil(ms / 1000));
  const m = Math.floor(total / 60);
  const sec = String(total % 60).padStart(2, '0');
  return `${m}:${sec}`;
}
function renderPhaseTimer(session) {
  const left = timeRemainingText(session);
  if (!left) return '';
  return `<span class="status-pill timer-pill">${left}</span>`;
}
function renderPlayerToolbar(d) {
  return html`<div class="player-toolbar">
    <button class="btn" data-open-info="status">Status</button>
    <button class="btn" data-open-info="group">Group Status</button>
    <button class="btn" data-open-info="players">Players</button>
  </div>`;
}
function renderInfoModal(d) {
  return html`<div class="info-modal" id="infoModal" hidden>
    <div class="info-modal__card">
      <div class="header"><h2 id="infoModalTitle">Status</h2><button class="btn" id="closeInfoModal">Close</button></div>
      <div id="infoModalBody"></div>
    </div>
  </div>`;
}
function infoPanelContent(type, d) {
  if (type === 'status') return html`<div class="stat-list">${statCells(d.player.stats, true)}</div><div class="divider"></div><div class="role-card"><h3>${esc(d.player.role?.name || 'Role pending')}</h3><p>${esc(d.player.role?.description || 'Waiting for role assignment.')}</p><p><strong>Status:</strong> ${esc(d.player.status)}</p></div>`;
  if (type === 'group') return html`<div class="stat-list">${Object.entries(d.groupStats || {}).map(([k,v]) => statCell(k, v, true, true)).join('')}</div>`;
  if (type === 'players') return html`<div class="players">${d.players.map(p => html`<div class="player-row"><div><strong>${esc(p.alias)}</strong><small>${esc(p.role?.name || 'Role hidden')} · ${esc(p.status)}</small></div></div>`).join('')}</div>`;
  return '';
}
function bindInfoModal(d) {
  const modal = document.getElementById('infoModal');
  const title = document.getElementById('infoModalTitle');
  const body = document.getElementById('infoModalBody');
  document.querySelectorAll('[data-open-info]').forEach(btn => btn.addEventListener('click', () => {
    const type = btn.dataset.openInfo;
    title.textContent = type === 'status' ? 'Status' : type === 'group' ? 'Group Status' : 'Players';
    body.innerHTML = infoPanelContent(type, d);
    modal.hidden = false;
  }));
  document.getElementById('closeInfoModal')?.addEventListener('click', () => { modal.hidden = true; });
  ...
  app.innerHTML = html`
    <section class="hero">
      <div class="title-card">
        <div class="kicker">Host Screen</div>
        <h1>Host Control</h1>
        <div class="panel soft">
          ...
          <div class="divider"></div>
          <button class="btn" id="resumeBtn">Resume Host Session</button>
        </div>
        <div class="divider"></div>
        <h2>Episodes</h2>
        <div class="grid three">${episodeCards()}</div>
...
function renderPlayerHome() {
  ...
  const joinShare = urlCodeValue ? renderJoinQrPanel({ session: { code: urlCodeValue.toUpperCase() } }, true) : '';
  app.innerHTML = html`
    <section class="hero">
      <div class="title-card player-home-card">
        <div class="kicker">Player Screen</div>
        <h1>Join Game</h1>
        <p class="tagline">Enter the code from the host and step into the episode.</p>
        ${joinShare}
        ${renderHowToPlayInstructions('player')}
        <div class="panel soft">
          <h2>Player Entry</h2>
          ...
          <div class="divider"></div>
          <button class="btn" id="resumeBtn">Resume Player Session</button>
        </div>
        <div class="divider"></div>
...
function renderHost() {
  ...
  app.innerHTML = html`
    <section class="grid host">
      <div class="grid">
        <div class="panel">
          <div class="header">
            <div><div class="kicker">Host Control · ${esc(e.subtitle || '')}</div><h2>${esc(e.title)}</h2><p class="muted">${esc(s.venueName)} · Game Code <span class="session-code">${esc(s.code)}</span></p><p class="muted"><strong>Scheduled:</strong> ${esc(formatSchedule(s))}</p></div>
            <div class="phase-stack"><span class="status-pill phase-pill">${esc(phaseLabels[s.phase] || s.phase)}</span>${renderPhaseTimer(s)}</div>
          </div>
          <p class="tagline">${esc(e.tagline)}</p>
          <div class="host-controls"><button class="btn primary" id="advanceBtn">${hostAdvanceLabel(d)}</button><button class="btn" id="autoRunBtn">${s.autoRun ? 'Auto-Run On' : 'Auto-Run Off'}</button><button class="btn" id="copyBtn">Copy Join Link</button><button class="btn danger" id="resetBtn">Reset Session</button><button class="btn" id="homeBtn">Leave</button></div>
          <div class="divider"></div>
          ${renderJoinQrPanel(d)}
  ...
  document.getElementById('autoRunBtn')?.addEventListener('click', async () => { try { await api(`/api/sessions/${s.code}/host/auto-run`, { method:'POST', body:{ hostKey: state.hostKey, autoRun: !s.autoRun } }); await fetchState(); } catch (err) { showToast(err.message); } });
...
function renderPlayer() {
  ...
  app.innerHTML = html`<section class="player-game">
    <div class="panel player-topbar">
      <div class="header">
        <div><div class="kicker">${esc(player.alias)} · ${esc(e.title)}</div><h2>${s.currentRound ? `Round ${s.currentRound}${round ? ': ' + esc(round.title) : ''}` : 'Lobby'}</h2><p class="muted">Code <span class="session-code">${esc(s.code)}</span> · <strong>Scheduled:</strong> ${esc(formatSchedule(s))}</p></div>
        <div class="phase-stack"><span class="status-pill phase-pill">${esc(phaseLabels[s.phase] || s.phase)}</span>${renderPhaseTimer(s)}</div>
      </div>
      ${player.role ? html`<div class="role-card compact-role"><h3>${esc(player.role.name)}</h3><p><strong>${esc(player.role.ability)}:</strong> ${esc(player.role.description)}</p></div>` : `<p class="muted">Waiting for role assignment.</p>`}
      ${renderPlayerToolbar(d)}
    </div>
    <div class="grid player-main">
      <div class="grid">${renderPlayerPhase(d)}${renderPlayerChat(d)}</div>
    </div>
    ${renderInfoModal(d)}
  </section>`;
  bindPlayerEvents(d);
...
loadEpisodes().then(() => {
  if (urlCode && routeMode === 'player' && localStorage.getItem('is_code') !== urlCode.toUpperCase()) {
    state.view = 'home';
    renderHome();
