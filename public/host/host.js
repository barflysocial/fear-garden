...
async function createSession() {
  ...
  const state = await api('/api/sessions', { method: 'POST', body: { tableName, truthPackId, playerCap, eventDate, eventTime, eventDurationMinutes, eventStatus } });
  upsertSession(state);
    ...
    <article class="tableCard">
      <div class="row">
        <div>
          ...
          <p class="mini">${escapeHtml(s.truthPackTitle)} · ${escapeHtml(s.difficultyLabel || s.difficulty || '')} · ${escapeHtml(eventLine(s))} · Detective Mode · Unified Evidence</p>
        </div>
