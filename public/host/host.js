...
async function loadAll() {
  ...
  $('truthPackSelect').innerHTML = packs.map(p => `<option value="${p.id}" ${p.id === 'fear-garden-rookie' ? 'selected' : ''}>${escapeHtml(p.gameTitle || 'Fear Garden')} — ${escapeHtml(p.levelLabel || p.difficulty || p.title)}</option>`).join('');
...
async function saveDemoCode() {
  ...
  try {
    ...
    alert('Demo access code saved.');
    await loadAll();
  } catch (err) {
    alert(err.message || 'Could not save demo access code.');
  }
}
...
async function resetDemoSession() {
  ...
  try {
    ...
    alert(`Demo reset complete. Fresh demo session: ${result.demoSessionCode || 'created'}.`);
    await loadAll();
  } catch (err) {
    alert(err.message || 'Could not reset demo session.');
  }
}
...
async function deleteDemoSessions() {
  ...
  try {
    ...
    alert(`Deleted ${result.deleted || 0} demo session${Number(result.deleted || 0) === 1 ? '' : 's'}.`);
    await loadAll();
  } catch (err) {
    alert(err.message || 'Could not delete demo sessions.');
  }
}

async function createSession() {
  const tableName = $('tableName').value.trim() || 'Table';
  const truthPackId = $('truthPackSelect').value || 'fear-garden-rookie';
  ...
  const eventDurationMinutes = Math.max(15, Math.min(240, Number($('eventDurationMinutes')?.value || 45)));
  const eventStatus = $('eventStatus')?.value || 'open';
  const eventType = $('eventType')?.value || 'paid';
  const sharedAccessCode = $('sharedAccessCode')?.value || '';
  const state = await api('/api/sessions', { method: 'POST', body: { tableName, truthPackId, playerCap, eventDate, eventTime, eventDurationMinutes, eventStatus, eventType, sharedAccessCode } });
  upsertSession(state);
...
function syncEventTypeFields() {
  const type = $('eventType')?.value || 'paid';
  const shared = $('sharedAccessCode');
  if (!shared) return;
  shared.disabled = type !== 'free';
  shared.placeholder = type === 'free' ? 'Example: FEARGARDEN' : 'Used for free events only';
}

function eventTypeLabel(s) {
  return (s.eventType || 'paid') === 'free' ? 'Free Event · Shared Code' : 'Paid Event · Unique Codes';
}
...
function rsvpHtml(s) {
  ...
  const rows = rsvps.map(item => {
    ...
    const codeCell = isFree ? `<button class="secondary codeButton" onclick="copyText('${escapeHtml(s.sharedAccessCode || code || '')}')">${escapeHtml(s.sharedAccessCode || code || 'Shared Code')}</button>` : (code ? `<button class="secondary codeButton" onclick="copyText('${escapeHtml(code)}')">${escapeHtml(code)}</button>` : '<span class="muted">Not assigned</span>');
    const paidCell = isFree ? '<span class="pill good">Free</span>' : `<button class="secondary compact" onclick="toggleRsvpPaid('${s.sessionCode}','${escapeHtml(item.id)}',${item.paid ? 'false' : 'true'})">${paidText}</button>`;
    const actionCell = isFree ? '<span class="muted">Shared code</span>' : `<button class="secondary compact" onclick="assignRsvpCode('${s.sessionCode}','${escapeHtml(item.id)}')">Assign Code</button>`;
    return `<tr>
...
function tableHtml(s) {
  ...
  return `
    <article class="tableCard">
      ...
      <div class="card" style="box-shadow:none"><h3>Check-In QR</h3>${checkInHtml(s)}</div>
      ...
      <div class="card" style="box-shadow:none"><h3>RSVP List</h3><p class="mini">Each RSVP equals one player spot. Free events use the shared code; paid events require one assigned code per player after payment.</p>${rsvpHtml(s)}</div>
