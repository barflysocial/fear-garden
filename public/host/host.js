...
if ($('addSponsorAdBtn')) $('addSponsorAdBtn').onclick = addSponsorAd;
if ($('clearSponsorFormBtn')) $('clearSponsorFormBtn').onclick = clearSponsorForm;
if ($('sponsorSessionSelect')) $('sponsorSessionSelect').onchange = renderSponsorManager;
...
function resetCreateForm() {
  ['tableName','playerCap','eventDate','eventTime','eventDurationMinutes','sharedAccessCode'].forEach(id => { if ($(id)) $(id).value = ''; });
  if ($('truthPackSelect')) $('truthPackSelect').value = '';
  if ($('eventType')) $('eventType').value = '';
  if ($('ticketPrice')) $('ticketPrice').value = '';
  if ($('eventStatus')) $('eventStatus').value = '';
...
async function loadAll() {
  ...
  $('truthPackSelect').innerHTML = '<option value="">Choose story chapter</option>' + packs.map(p => `<option value="${p.id}">${escapeHtml(p.gameTitle || 'Fear Garden')} — ${escapeHtml(p.levelLabel || p.difficulty || p.title)}</option>`).join('');
...
async function resetDemoSession() {
  if (!confirm('Reset Demo Mode now? This removes old demo sessions and creates one clean fresh demo session.')) return;
  try {
...
async function deleteDemoSessions() {
  if (!confirm('Delete all demo sessions from the dashboard and database? The demo access code will remain active.')) return;
  try {
...
async function createSession() {
  ...
  if (!tableName || !truthPackId || !playerCapRaw || !eventDate || !eventTime || !durationRaw || !eventStatus || !eventType) {
    hostNotify('Complete all Create Session fields before creating a session.', 'error');
    return;
  }
  if (eventType === 'paid' && !ticketPrice) {
    hostNotify('Enter a ticket price for paid events.', 'error');
    return;
  }
  const playerCap = Math.max(1, Math.min(100, Number(playerCapRaw)));
  const eventDurationMinutes = Math.max(15, Math.min(240, Number(durationRaw)));
  try {
    const state = await api('/api/sessions', { method: 'POST', body: { tableName, truthPackId, playerCap, eventDate, eventTime, eventDurationMinutes, eventStatus, eventType, ticketPrice, sharedAccessCode, allowLateCheckIn, scheduledStartAt, autoStartEnabled } });
    upsertSession(state);
...
function hostPrimaryAction(s) {
  ...
  if (s.status === 'briefing') return `<button class="good" onclick="sendOpeningNarration('${s.sessionCode}')">Send Opening</button>`;
...
function sponsorTimingLabel(timing) {
  return ({
    waiting_before_briefing: 'Waiting Room / Before Briefing',
    after_round_2_checkpoint: 'After Round 2 Checkpoint',
    after_round_4_checkpoint: 'After Round 4 Checkpoint',
    before_final_accusation: 'Before Final Accusation',
    case_closed: 'Case Closed Screen'
  })[timing] || 'Sponsor Break';
}

function clearSponsorForm() {
  if ($('sponsorTitle')) $('sponsorTitle').value = '';
  if ($('sponsorMessage')) $('sponsorMessage').value = '';
  if ($('sponsorTiming')) $('sponsorTiming').value = 'waiting_before_briefing';
}

function renderSponsorManager() {
  const select = $('sponsorSessionSelect');
  const list = $('sponsorManagerList');
  if (!select || !list) return;
  const current = select.value;
  select.innerHTML = sessions.length
    ? sessions.map(s => `<option value="${escapeHtml(s.sessionCode)}">${escapeHtml(eventLine(s))} — ${escapeHtml(s.tableName)} · ${escapeHtml(s.sessionCode)}</option>`).join('')
    : '<option value="">Create a session first</option>';
  if (current && sessions.some(s => s.sessionCode === current)) select.value = current;
  const session = sessions.find(s => s.sessionCode === select.value) || sessions[0];
  if (session && select.value !== session.sessionCode) select.value = session.sessionCode;
  if (!session) {
    list.innerHTML = '<p class="muted">No sessions available yet.</p>';
    return;
  }
  const ads = session.sponsorAds || [];
  if (!ads.length) {
    list.innerHTML = '<p class="muted">No sponsor popups have been added to this session yet.</p>';
    return;
  }
  list.innerHTML = ads.map(ad => `
    <div class="feedItem sponsorAdItem">
      <div class="time">${escapeHtml(sponsorTimingLabel(ad.timing))}</div>
      <h4>${escapeHtml(ad.title || 'Sponsor Message')}</h4>
      <p>${escapeHtml(ad.message || '')}</p>
      <div class="statusPills"><span class="pill ${ad.enabled !== false ? 'good' : ''}">${ad.enabled !== false ? 'Enabled' : 'Disabled'}</span></div>
      <div class="actions compactButtons" style="margin-top:10px">
        <button class="secondary compact" onclick="toggleSponsorAd('${session.sessionCode}','${escapeHtml(ad.id)}',${ad.enabled !== false ? 'false' : 'true'})">${ad.enabled !== false ? 'Disable' : 'Enable'}</button>
        <button class="danger compact" onclick="deleteSponsorAd('${session.sessionCode}','${escapeHtml(ad.id)}')">Delete</button>
      </div>
    </div>
  `).join('');
}

async function addSponsorAd() {
  const sessionCode = $('sponsorSessionSelect')?.value || '';
  const title = $('sponsorTitle')?.value || '';
  const message = $('sponsorMessage')?.value || '';
  const timing = $('sponsorTiming')?.value || 'waiting_before_briefing';
  if (!sessionCode) { hostNotify('Choose a session first.', 'error'); return; }
  if (!message.trim()) { hostNotify('Enter an ad message.', 'error'); return; }
  try {
    const state = await api(`/api/sessions/${sessionCode}/sponsors`, { method: 'POST', body: { title, message, timing, enabled: true } });
    upsertSession(state);
    render();
    clearSponsorForm();
    hostNotify('Sponsor popup added.');
  } catch (err) {
    hostNotify(err.message || 'Could not add sponsor popup.', 'error');
  }
}

window.toggleSponsorAd = async (sessionCode, adId, enabled) => {
  try {
    const state = await api(`/api/sessions/${sessionCode}/sponsors/${adId}`, { method: 'PATCH', body: { enabled } });
    upsertSession(state);
    render();
...
window.deleteSponsorAd = async (sessionCode, adId) => {
  if (!confirm('Delete this sponsor popup?')) return;
  try {
    const state = await api(`/api/sessions/${sessionCode}/sponsors/${adId}`, { method: 'DELETE' });
    upsertSession(state);
    render();
    hostNotify('Sponsor popup deleted.');
  } catch (err) { hostNotify(err.message || 'Could not delete sponsor popup.', 'error'); }
};

function sponsorAdSummaryHtml(s) {
  const ads = s.sponsorAds || [];
  if (!ads.length) return '<p class="muted">No automated sponsor popups for this session.</p>';
  return ads.map(ad => `<div class="feedItem"><div class="time">${escapeHtml(sponsorTimingLabel(ad.timing))}</div><h4>${escapeHtml(ad.title || 'Sponsor Message')}</h4><p>${escapeHtml(ad.message || '')}</p><span class="pill ${ad.enabled !== false ? 'good' : ''}">${ad.enabled !== false ? 'Enabled' : 'Disabled'}</span></div>`).join('');
}
  ...
  return `<div class="checkinBox">
    <div>
      ...
      <div class="actions compactButtons" style="margin-top:10px">
        ...
        <button class="secondary" onclick="window.open('${escapeHtml(link)}','_blank')">Open Check-In Page</button>
      </div>
    </div>
...
function tableHtml(s) {
  ...
  const help = (s.helpRequests || []).slice(0, 4).map(h => `<div class="feedItem"><div class="time">${new Date(h.createdAt).toLocaleTimeString()}</div><h4>${escapeHtml(h.playerName)}</h4>${h.text ? `<p>${escapeHtml(h.text)}</p>` : ''}</div>`).join('') || '<p class="muted">No help requests.</p>';
...
window.startTable = async code => {
  try {
    ...
    hostNotify('Briefing has started. Players in the waiting room will enter the case setup.');
  } catch (err) { hostNotify(err.message || 'Could not start game.', 'error'); }
};
window.resetTable = async code => {
  if (!confirm('Reset this table? This will clear players, answers, messages, results, timer state, and activity.')) return;
  try { await api(`/api/sessions/${code}/reset`, { method: 'POST' }); hostNotify('Session has been reset.'); }
...
window.deleteSession = async code => {
  ...
  try {
    ...
    render();
    hostNotify('Session has been deleted.');
  } catch (err) { hostNotify(err.message || 'Could not delete session.', 'error'); }
};
window.revealCase = async code => { try { await api(`/api/sessions/${code}/reveal`, { method: 'POST' }); hostNotify('Reveal has been triggered.'); } catch (err) { hostNotify(err.message || 'Could not reveal case.', 'error'); } };
...
window.copyAllCodes = async code => {
  ...
  try { await navigator.clipboard.writeText(lines.join('\n')); hostNotify('Access codes copied.'); } catch (_err) { alert(lines.join('\n')); }
};
...
window.deleteRsvp = async (sessionCode, rsvpId) => {
  const ok = confirm('Delete this RSVP? This frees the RSVP spot and unassigns that access code.');
  if (!ok) return;
