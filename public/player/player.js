...
function enqueueClueDialogs(clues, sessionCode) {
  ...
  for (const group of byApp.values()) {
    enqueueDialog({
      ...
      text: `New evidence has unlocked. Open the ${group.appLabel} icon to review it.`,
      viewLabel: `Open ${group.appLabel}`,
      viewAction: () => window.openApp(group.appKey),
      ackType: 'clues',
      ackValues: group.ids
    });
  }
...
function render() {
  ...
  if (isBriefing) $('storyCountdown').textContent = fmt(state.briefingRemainingSec);
...
function renderReveal() {
  ...
  if (result) {
    ...
    $('resultSummary').innerHTML = '<p class="muted">Your result will appear here when the case reveal begins.</p>';
