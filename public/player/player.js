...
['rsvpDateFilter','rsvpTimeFilter','rsvpMysteryFilter','rsvpLevelFilter'].forEach(id => { if ($(id)) $(id).addEventListener('change', renderRsvpBrowser); });
...
function buildRsvpFilters() {
  fillFilter('rsvpDateFilter', rsvpSessions.map(s => s.dateLabel || 'Date TBD'), 'All Dates');
  fillFilter('rsvpTimeFilter', rsvpSessions.map(s => s.timeLabel || 'Time TBD'), 'All Times');
  fillFilter('rsvpMysteryFilter', rsvpSessions.map(s => s.mysteryTitle || s.mystery || 'Fear Garden'), 'All Mysteries');
  fillFilter('rsvpLevelFilter', rsvpSessions.map(s => s.levelLabel || s.difficultyLabel || s.difficulty || 'Skill Level TBD'), 'All Skill Levels');
}
...
function renderRsvpBrowser() {
  ...
  const time = $('rsvpTimeFilter')?.value || '';
  const mystery = $('rsvpMysteryFilter')?.value || '';
  const level = $('rsvpLevelFilter')?.value || '';
  const filtered = rsvpSessions.filter(item => {
    return (!date || item.dateLabel === date) &&
      (!time || item.timeLabel === time) &&
      (!mystery || (item.mysteryTitle || item.mystery) === mystery) &&
      (!level || (item.levelLabel || item.difficultyLabel || item.difficulty) === level);
  });
  ...
  if (!filtered.length) {
    list.innerHTML = '<p class="muted">No investigations match those filters. Try another date, time, mystery, or skill level.</p>';
    msg.textContent = 'Choose from the available sessions below.';
    return;
  }
  const grouped = groupBy(filtered, item => item.dateLabel || 'Date TBD');
  list.innerHTML = Object.entries(grouped).map(([dateLabel, items]) => `
    <div class="showtimeDateGroup">
      <h3>${escapeHtml(dateLabel)}</h3>
      ${items.map(showtimeCardHtml).join('')}
    </div>`).join('');
  msg.textContent = 'Tap a showtime to reserve your detective spot.';
...
function showtimeCardHtml(item) {
  ...
  return `<article class="showtimeCard">
    <div>
      ...
      <div class="statusPills"><span class="pill ${soldOut ? '' : 'good'}">${escapeHtml(status)}</span><span class="pill">${escapeHtml(String(item.eventDurationMinutes || 45))} min</span><span class="pill">${escapeHtml(item.tableName || 'Session')}</span></div>
    </div>
...
function selectRsvpSession(code) {
  ...
  $('selectedSessionCard').innerHTML = `<div class="time">Selected Showtime</div>
    ...
    <p class="mini">${escapeHtml(String(item.eventDurationMinutes || 45))}-minute session · ${escapeHtml(String(item.seatsAvailable ?? item.spotsAvailable ?? 0))} seats left out of ${escapeHtml(String(item.playerCap || 25))}</p>`;
...
function showRsvpBrowser() {
  ...
  $('rsvpMessage').textContent = 'Choose an available date, time, mystery, and skill level.';
}
...
async function submitRsvp() {
  ...
  try {
    ...
    msg.innerHTML = '✅ RSVP saved. Your detective spot is reserved for this showtime. You cannot reserve another investigation that overlaps this time window. Pay the host or event page to receive your personal access code.';
  } catch (err) {
