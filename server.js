...
const phaseDurationsByMode = {
  ...
  standard: { story_intro: 50, role_reveal: 20, public_event: 45, private_scenario: 45, public_discussion: 105, private_alliance: 105, choice_lock: 75, round_results: 55, final_intro: 45, final_choice: 60 },
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      if (parts[0] === 'api' && parts[1] === 'sessions' && parts[2]) {
        ...
        if (req.method === 'POST' && parts[3] === 'join') {
          ...
          if (!/^[\p{L}][\p{L}\p{M}'’.-]*(?: [\p{L}][\p{L}\p{M}'’.-]*)?$/u.test(firstName)) throw new Error('Enter a valid first name.');
