...
const { initPersistence, saveSession, persistenceInfo } = require('./db');
...
const phaseDurations = {
  quick: { role_reveal: 8000, public_event: 12000, private_scenario: 18000, public_discussion: 45000, private_alliance: 45000, choice_lock: 35000, round_results: 15000, final_intro: 15000, final_choice: 45000 },
  standard: { role_reveal: 12000, public_event: 20000, private_scenario: 30000, public_discussion: 90000, private_alliance: 90000, choice_lock: 60000, round_results: 30000, final_intro: 25000, final_choice: 60000 },
  long: { role_reveal: 20000, public_event: 30000, private_scenario: 45000, public_discussion: 120000, private_alliance: 120000, choice_lock: 90000, round_results: 45000, final_intro: 35000, final_choice: 90000 }
};
...
function getPhaseDuration(session, phase = session.phase) {
  const mode = phaseDurations[session.mode] || phaseDurations.standard;
  return mode[phase] || 30000;
}
function setSessionPhase(session, phase) {
  session.phase = phase;
  session.phaseStartedAt = now();
  const duration = getPhaseDuration(session, phase);
  session.phaseDurationMs = duration;
  session.phaseEndsAt = ['ending', 'lobby'].includes(phase) ? null : new Date(Date.now() + duration).toISOString();
}
...
function migrateSession(session) {
  ...
  if (typeof session.autoRun !== 'boolean') session.autoRun = true;
  session.phaseStartedAt = session.phaseStartedAt || null;
  session.phaseEndsAt = session.phaseEndsAt || null;
  session.phaseDurationMs = session.phaseDurationMs || 0;
...
function publicSession(session) {
  ...
  return {
    ...
    phaseDurationMs: session.phaseDurationMs || 0,
    serverNow: now(),
...
function startGame(session) {
  ...
  setSessionPhase(session, 'role_reveal');
...
function advancePhase(session) {
  ...
  if (session.status === 'final') {
    if (session.phase === 'final_intro') setSessionPhase(session, 'final_choice');
  ...
  if (session.phase === 'round_results') {
    ...
    session.currentRound += 1;
    setSessionPhase(session, 'public_event');
  ...
  if (idx >= 0 && idx < phaseOrder.length - 1) {
    setSessionPhase(session, phaseOrder[idx + 1]);
...
function startFinal(session) {
  ...
  setSessionPhase(session, 'final_intro');
  const ep = getSessionEpisode(session);
...
function resolveRound(session) {
  ...
  setSessionPhase(session, 'round_results');
}
...
function resolveFinal(session) {
  ...
  setSessionPhase(session, 'ending');
  session.endedAt = now();
}
...
async function processAutomatedSessions() {
  ...
  try {
    ...
    for (const session of sessions.values()) {
      migrateSession(session);
      if (!session.autoRun) continue;
      let changed = false;
      if (session.status === 'lobby' && session.scheduledAt && new Date(session.scheduledAt).getTime() <= nowMs && session.players.length > 0) {
        startGame(session);
        changed = true;
      } else if ((session.status === 'active' || session.status === 'final') && session.phaseEndsAt && new Date(session.phaseEndsAt).getTime() <= nowMs) {
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      if (parts[0] === 'api' && parts[1] === 'sessions' && parts[2]) {
        ...
        if (req.method === 'GET' && parts[3] === 'qr.svg') {
          ...
          const data = encodeURIComponent(`${proto}://${host}/play?code=${code}`);
          res.writeHead(302, { Location: `https://api.qrserver.com/v1/create-qr-code/?size=240x240&margin=12&data=${data}`, 'Cache-Control': 'no-store' });
          return res.end();
        }
        ...
        if (req.method === 'POST' && parts[3] === 'host' && parts[4] === 'auto-run') {
          const body = await parseBody(req); requireHost(session, body.hostKey);
          session.autoRun = body.autoRun !== false;
          if (session.autoRun && session.status !== 'ended' && session.phase !== 'lobby') setSessionPhase(session, session.phase);
          await saveSession(session);
        ...
        if (req.method === 'POST' && parts[3] === 'host' && parts[4] === 'reset') {
          ...
          const newSession = createSession({ venueName: session.venueName, mode: session.mode, maxPlayers: session.maxPlayers, episodeId: session.episodeId, scheduledDate: session.scheduledDate, scheduledTime: session.scheduledTime, autoRun: session.autoRun });
...
async function startServer() {
  ...
  setInterval(processAutomatedSessions, 1000);
}
