...
const { initPersistence, saveSession, persistenceInfo, closePersistence } = require('./db');
...
function rateLimit(req, res) {
  ...
  const bucket = requestBuckets.get(key) || { count: 0, resetAt: nowMs + RATE_LIMIT_WINDOW_MS };
  if (nowMs > bucket.resetAt) { bucket.count = 0; bucket.resetAt = nowMs + RATE_LIMIT_WINDOW_MS; }
  bucket.count += 1;
  requestBuckets.set(key, bucket);
  if (bucket.count > RATE_LIMIT_MAX) {
    sendJson(res, { error: 'Too many requests. Try again in a minute.' }, 429, { 'Retry-After': String(Math.ceil((bucket.resetAt - nowMs) / 1000)) });
    return false;
  }
  return true;
}
function pruneRateBuckets() {
  const nowMs = Date.now();
  for (const [key, bucket] of requestBuckets.entries()) if (nowMs > bucket.resetAt + RATE_LIMIT_WINDOW_MS) requestBuckets.delete(key);
}
...
function securityHeaders(extra = {}) {
  ...
  if (APP_ORIGIN) connectSrc.push(APP_ORIGIN);
  return {
    ...
    'Content-Security-Policy': `default-src 'self'; img-src 'self' data:; script-src 'self'; style-src 'self' 'unsafe-inline'; connect-src ${connectSrc.join(' ')}; base-uri 'self'; frame-ancestors 'none'; form-action 'self'`,
    ...extra
  };
}
function absoluteJoinUrl(req, code) {
  const proto = req.headers['x-forwarded-proto'] || (req.socket.encrypted ? 'https' : 'http');
  const host = req.headers['x-forwarded-host'] || req.headers.host || 'localhost';
  return `${proto}://${host}/play?code=${encodeURIComponent(code || '')}`;
}
async function makeQrSvg(data) {
  try {
    const QRCode = require('qrcode');
    return QRCode.toString(data, { type: 'svg', margin: 1, width: 280, errorCorrectionLevel: 'M' });
  } catch (err) {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="280" height="280" viewBox="0 0 280 280"><rect width="280" height="280" fill="#fff"/><text x="140" y="125" text-anchor="middle" font-family="Arial" font-size="16" fill="#111">QR package missing</text><text x="140" y="152" text-anchor="middle" font-family="Arial" font-size="12" fill="#111">Run npm install</text></svg>`;
  }
}
function sendSvg(res, svg, status = 200) {
  res.writeHead(status, securityHeaders({ 'Content-Type': 'image/svg+xml; charset=utf-8', 'Content-Length': Buffer.byteLength(svg), 'Cache-Control': 'no-store' }));
  res.end(svg);
}
...
function cleanTime(value) {
  ...
  return hh >= 0 && hh <= 23 && mm >= 0 && mm <= 59 ? v : '';
}
function scheduledAtFrom(date, time) {
...
function createSession(body = {}) {
  ...
  const session = {
    ...
    episodeId,
    venueName: cleanText(body.venueName || ep.venueDefault || ep.title, 60),
    mode: body.mode || 'standard',
    scheduledDate: cleanDate(body.scheduledDate),
    scheduledTime: cleanTime(body.scheduledTime),
    scheduledAt: scheduledAtFrom(cleanDate(body.scheduledDate), cleanTime(body.scheduledTime)),
    maxPlayers,
    status: 'lobby',
    phase: 'lobby',
    currentRound: 0,
...
function migrateSession(session) {
  ...
  session.scheduledDate = cleanDate(session.scheduledDate);
  session.scheduledTime = cleanTime(session.scheduledTime);
  session.scheduledAt = session.scheduledAt || scheduledAtFrom(session.scheduledDate, session.scheduledTime);
...
function advancePhase(session) {
  ...
  if (session.status === 'final') {
    if (session.phase === 'final_intro') session.phase = 'final_choice';
...
function sendJson(res, data, status = 200, extraHeaders = {}) {
  ...
  res.writeHead(status, securityHeaders({ 'Content-Type': 'application/json; charset=utf-8', 'Content-Length': Buffer.byteLength(json), 'Cache-Control': 'no-store', ...extraHeaders }));
  res.end(json);
}
...
function requireHost(session, key) { if (!safeEqual(key, session.hostKey)) throw new Error('Invalid host key.'); }
...
const server = http.createServer(async (req, res) => {
  try {
    if (!['GET','POST','HEAD'].includes(req.method)) return sendError(res, 'Method not allowed.', 405);
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      if (['GET','HEAD'].includes(req.method) && /^\/api\/sessions\/[^/]+\/qr\.svg$/.test(url.pathname)) {
        const code = url.pathname.split('/')[3].toUpperCase();
        const session = requireSession(code);
        const svg = await makeQrSvg(absoluteJoinUrl(req, session.code));
      ...
      if (parts[0] === 'api' && parts[1] === 'sessions' && parts[2]) {
        ...
        if (req.method === 'GET' && parts[3] === 'state') {
          ...
          if (hostKey && safeEqual(hostKey, session.hostKey)) return sendJson(res, hostView(session));
        ...
        if (req.method === 'POST' && parts[3] === 'join') {
          ...
          const alias = cleanText(body.alias, 24);
        ...
        if (req.method === 'POST' && parts[3] === 'host' && parts[4] === 'reset') {
          ...
          const oldCode = session.code, hostKey = session.hostKey;
          const newSession = createSession({ venueName: session.venueName, mode: session.mode, maxPlayers: session.maxPlayers, episodeId: session.episodeId, scheduledDate: session.scheduledDate, scheduledTime: session.scheduledTime });
          sessions.delete(newSession.code);
          newSession.code = oldCode; newSession.hostKey = hostKey; sessions.set(oldCode, newSession);
        ...
        if (req.method === 'POST' && parts[3] === 'players' && parts[4]) {
          ...
          if (action === 'alliance') {
            ...
            const name = cleanText(body.name || 'Secret Alliance', 32);
...
function serveStatic(req, res, pathname) {
  const rawPath = pathname === '/' || pathname === '/host' || pathname === '/play' ? '/index.html' : pathname;
  const decoded = decodeURIComponent(rawPath);
  const filePath = path.normalize(path.join(PUBLIC_DIR, decoded));
  const publicRoot = path.normalize(PUBLIC_DIR + path.sep);
  if (!filePath.startsWith(publicRoot)) return sendError(res, 'Forbidden', 403);
  fs.readFile(filePath, (err, data) => {
    if (err) {
      fs.readFile(path.join(PUBLIC_DIR, 'index.html'), (err2, data2) => {
        if (err2) return sendError(res, 'Not found', 404);
        const headers = securityHeaders({ 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' });
        res.writeHead(200, headers);
        if (req.method !== 'HEAD') res.end(data2); else res.end();
      });
      return;
    }
    const ext = path.extname(filePath).toLowerCase();
    const types = { '.html': 'text/html; charset=utf-8', '.js': 'application/javascript; charset=utf-8', '.css': 'text/css; charset=utf-8', '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.svg': 'image/svg+xml; charset=utf-8', '.ico': 'image/x-icon' };
    const immutable = /\/titlecards\//.test(filePath) || ['.png','.jpg','.jpeg','.svg','.ico'].includes(ext);
    res.writeHead(200, securityHeaders({ 'Content-Type': types[ext] || 'application/octet-stream', 'Cache-Control': immutable ? 'public, max-age=31536000, immutable' : 'no-store' }));
...
function shutdown(signal) {
  ...
  server.close(async () => { try { await closePersistence(); } finally { process.exit(0); } });
